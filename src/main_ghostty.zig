...
pub fn main(minimal: std.process.Init.Minimal) !MainReturn {
    ...
    if (comptime build_config.app_runtime == .none) {
        ...
        try stdout.print("Usage: ghosttal +<action> [flags]\n\n", .{});
        try stdout.print(
            \\This is the Ghosttal helper CLI that accompanies the graphical Ghosttal app.
            \\To launch the terminal directly, please launch the graphical app
            \\(i.e. Ghosttal.app on macOS). This CLI can be used to perform various
            \\actions such as inspecting the version, listing fonts, etc.
            \\
            \\On macOS, the terminal can also be launched using `open -na Ghosttal.app`,
            \\or `open -na Ghosttal.app --args --foo=bar --baz=qux` to pass arguments.
            \\
